...
public class BrainManager : MonoBehaviour
{
	...
	public Material roundedCubeMat;
	...
	public void SetFourthGlyphVisibility (bool v)
	{
		this.SetGlyphVisibility (3, v);
	}
	...
	private void CreateVoxel (string line)
	{
		...
		{ 
			...
			Vector2[] uvs = mesh.uv;
			float v = this.fRange.ToVisRange (glyph.f);
		...
		// fourth glyph
		glyph = voxel.innerGlyphs [3];
		glyph.Initialize (fa, Vector3.zero, 0);
		{ 
			// update the UVs of the mesh
			Mesh mesh = glyph.GetComponent<MeshFilter> ().mesh;
			Vector2[] uvs = mesh.uv;
			float v = this.faRange.ToVisRange (fa);
			for (int i = 0; i < uvs.Length; i++) {
				uvs [i].x = v;
				uvs [i].y = 0;
			}
