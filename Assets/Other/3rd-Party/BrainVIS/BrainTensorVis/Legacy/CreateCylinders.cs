using UnityEngine;
using System;
using System.Collections;
using System.Globalization;

/**
  x  y  z   f1   n1(1)  n1(2)  n1(3)  R1  f2   n2(1)  n2(2)  n2(3)  R2

  x, y, z are the coordinates and the total matrix size is 80x96x64. 
  f1 is the volume fraction (density) of the cylinder1, 
  n1 is a 3x1 unit vector representing cylinder1 orientation. 
  R1 is the radius of cylinder1. 

  f2, n2 and R2 describe volume fraction (density), orientation and radius of the second cylinder.
  Length of both the cylinders are a contant = 1.7e3.
 */



public class CreateCylinders : MonoBehaviour
{

	[Tooltip ("Cylinder size, max 0.5")]
	public float clength = 0.4f;

	[Tooltip ("Min cylinder radius")]
	public float cminwidth = 0.05f;

	[Tooltip ("Max cylinder radius")]
	public float cmaxwidth = 0.3f;

	public Color f1LowColor = new Color (1f, 0.5f, 0.25f);
	public Color f1HighColor = new Color (1f, 1f, 1f);

	public Color f2LowColor = new Color (0.5f, 0.5f, 1f);
	public Color f2HighColor = new Color (1f, 1f, 1f);

	// Use this for initialization
	void Start ()
	{

		int xmax = 80;
		int ymax = 96;
		int zmax = 64;

		float[,,] f1 = new float[xmax, ymax, zmax];
		Vector3[,,] n1 = new Vector3[xmax, ymax, zmax];
		float[,,] R1 = new float[xmax, ymax, zmax];

		float[,,] f2 = new float[xmax, ymax, zmax];
		Vector3[,,] n2 = new Vector3[xmax, ymax, zmax];
		float[,,] R2 = new float[xmax, ymax, zmax];


		// load data
		string[] lines = System.IO.File.ReadAllLines ("Assets/two_cylinders_z32.txt");
		// skip the first line, which is the header
		for (int l = 1; l < lines.Length; l++) { 
			string[] values = lines [l].Split (new char[] { ' ', '\t' }, StringSplitOptions.RemoveEmptyEntries);
			int x = (int)(float.Parse (values [0], CultureInfo.InvariantCulture)) - 1;
			int y = (int)(float.Parse (values [1], CultureInfo.InvariantCulture)) - 1;
			int z = (int)(float.Parse (values [2], CultureInfo.InvariantCulture)) - 1;

			f1 [x, y, z] = float.Parse (values [3], CultureInfo.InvariantCulture);
			n1 [x, y, z] = new Vector3 (float.Parse (values [4], CultureInfo.InvariantCulture), float.Parse (values [5], CultureInfo.InvariantCulture), float.Parse (values [6], CultureInfo.InvariantCulture));
			R1 [x, y, z] = float.Parse (values [7], CultureInfo.InvariantCulture);

			f2 [x, y, z] = float.Parse (values [8], CultureInfo.InvariantCulture);
			n2 [x, y, z] = new Vector3 (float.Parse (values [9], CultureInfo.InvariantCulture), float.Parse (values [10], CultureInfo.InvariantCulture), float.Parse (values [11], CultureInfo.InvariantCulture));
			R2 [x, y, z] = float.Parse (values [12], CultureInfo.InvariantCulture);
		}


		// calc max values
		float fmax = 0.0f;
		float Rmax = 0.0f;
		for (int z = 0; z < zmax; z++) {
			for (int y = 0; y < ymax; y++) {
				for (int x = 0; x < xmax; x++) {
					if (f1 [x, y, z] > fmax) {
						fmax = f1 [x, y, z];
					}
					if (f2 [x, y, z] > fmax) {
						fmax = f2 [x, y, z];
					}
					if (R1 [x, y, z] > Rmax) {
						Rmax = R1 [x, y, z];
					}
					if (R2 [x, y, z] > Rmax) {
						Rmax = R2 [x, y, z];
					}
				}
			}
		}

		// setup gradients
		GradientAlphaKey[] gak = new GradientAlphaKey[2];
		gak [0].alpha = 1.0F;
		gak [0].time = 0.0F;
		gak [1].alpha = 0.0F;
		gak [1].time = 1.0F;

		Gradient g1 = new Gradient ();
		GradientColorKey[] gck1 = new GradientColorKey[2];
		gck1 [0].color = f1LowColor;
		gck1 [0].time = 0.0F;
		gck1 [1].color = f1HighColor;
		gck1 [1].time = 1.0F;
		g1.SetKeys (gck1, gak);

		Gradient g2 = new Gradient ();
		GradientColorKey[] gck2 = new GradientColorKey[2];
		gck2 [0].color = f2LowColor;
		gck2 [0].time = 0.0F;
		gck2 [1].color = f2HighColor;
		gck2 [1].time = 1.0F;
		g2.SetKeys (gck1, gak);

		
		// create geometry
		for (int z = 31; z < 32; z++) {
			for (int y = 0; y < ymax; y++) {
				for (int x = 0; x < xmax; x++) {
					GameObject glyph1 = GameObject.CreatePrimitive (PrimitiveType.Cylinder);
					glyph1.transform.position = new Vector3 ((float)x - (float)(xmax / 2), (float)y - (float)(ymax / 2), (float)0.0);
					glyph1.transform.rotation = Quaternion.FromToRotation (new Vector3 (0, 1, 0), n1 [x, y, z]);
					float w1 = (R1 [x, y, z] / Rmax) * (cmaxwidth - cminwidth) + cminwidth;
					glyph1.transform.localScale = new Vector3 (w1, clength, w1);
					Renderer rend1 = glyph1.GetComponent<Renderer> ();
					rend1.material.color = g1.Evaluate (f1 [x, y, z] / fmax); //new Color (1f, 0.5f, 0.25f);'

					GameObject glyph2 = GameObject.CreatePrimitive (PrimitiveType.Cylinder);
					glyph2.transform.position = new Vector3 ((float)x - (float)(xmax / 2), (float)y - (float)(ymax / 2), (float)0.0);
					glyph2.transform.rotation = Quaternion.FromToRotation (new Vector3 (0, 1, 0), n2 [x, y, z]);
					float w2 = (R2 [x, y, z] / Rmax) * (cmaxwidth - cminwidth) + cminwidth;
					glyph2.transform.localScale = new Vector3 (w2, clength, w2);
					Renderer rend2 = glyph2.GetComponent<Renderer> ();
					rend2.material.color = new Color (0.5f, 0.5f, 1f);
				}
			}
		}

	}
	
	// Update is called once per frame
	void Update ()
	{
	
	}
}
