...
public class DataGlyphRenderer : DataRenderer
{
    ...
    public override void RefreshDataRenderer()
    {
        ...
        if (pointData.GetNumberOfArrays() > arrayId)
        {
...

            {
                ...
                long numberOfComponents = dataArray.GetNumberOfComponents();

                print(arrayId + ": " + dataArray.GetName());
                if (dataArray.GetNumberOfComponents() != 3)
                {
                    print("Can't handle scalars... yet");
                    return;
                }
                ...
                float[] data = new float[w * h * d * numberOfComponents];
                ...
                dataBuffer = new ComputeBuffer((int)data.Length, sizeof(float) * (int)numberOfComponents);
                dataBuffer.SetData(data);

                print("Filling a texture of size " + w + "x" + h + "x" + d + "x" + numberOfComponents);


                for (int i = 0; i < numberOfTuples; i++)
                {
                    colorData[i].r = data[i * 3 + 0];
                    colorData[i].g = data[i * 3 + 1];
                    colorData[i].b = data[i * 3 + 2];
                    colorData[i].a = 1;
                ...
                _volumeBuffer = new Texture3D((int)(w), (int)(h), (int)(d), TextureFormat.RGBAFloat, false);
                _volumeBuffer.SetPixels(colorData);
                _volumeBuffer.Apply();

                glyphMaterial.SetTexture("_DataVolume", _volumeBuffer);

            }


            {
                vtkDataArray dataArray = pointData.GetArray(4);
                long numberOfTuples = dataArray.GetNumberOfTuples();
                long numberOfComponents = dataArray.GetNumberOfComponents();
                float[] data = new float[w * h * d * numberOfComponents];
                Color[] colorData = new Color[w * h * d];
                ...
                for (int i = 0; i < numberOfTuples; i++)
                {
                    ...
                    colorData[i].g = data[i ];
                    colorData[i].b = data[i ];
                    colorData[i].a = 1;
                ...
                _volumeBuffer.SetPixels(colorData);
                _volumeBuffer.Apply();
                glyphMaterial.SetTexture("_DataVolume2", _volumeBuffer);
            ...
            glyphMaterial.SetVector("_Dimensions", new Vector3(w, h, d));

            for (int c = 0; c < transform.childCount; c++)
            {
                transform.GetChild(c).GetComponent<MeshRenderer>().material = glyphMaterial;
            }
    ...
    public void Update()
    {
        glyphMaterial.SetMatrix("_ModelMatrix", transform.localToWorldMatrix);
        glyphMaterial.SetMatrix("_ModelMatrixInv", transform.worldToLocalMatrix);
        for (int c = 0; c < transform.childCount; c++)
        {
            transform.GetChild(c).GetComponent<MeshRenderer>().material = glyphMaterial;
